    ...
    public static class Algorithms
    {
        public static void fcfsAlgorithm(string userInput)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                //Calculate wait time: Later P wait time = all the former P burest time
                for (num = 0; num <= np - 1; num++)
                {
                    if (num == 0)
                    {
        ...
        {
            //Key Question: How to link the time to specifi PID? Not OOP. No object
            //Key items: PID->Process   Arrival Time    BurstTime   RemainingTime
            int np = Convert.ToInt16(userInput);
            double[] arrival = new double[np];
            double[] burst = new double[np];
            double[] remain = new double[np];

            //Critical moment: New Process Enter    One task complete
            //Perfomance indicators: WaitTime   TurnaroundTime
            ...
            //Answer: use an other array to indicate the sorted value belongs to which PID
            ...
            if (result == DialogResult.Yes) 
            {
                //Get Arrival and Burst. Burst == Remain -> Not execute yet
                for (i = 0; i < np; i++)
                {
                    string arrivalInput =
                            Microsoft.VisualBasic.Interaction.InputBox("Enter arrival time: ",
